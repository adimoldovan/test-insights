...
from allure.result import Result
...
class JSONReporter:
    ...
    def report(self):
        ...
        # run it twice because if the first found result is passed and doesn't already exists it will be ignored
        for path in self.paths_list:
            self.add_results_from_path(path)
    ...
    def add_results_from_path(self, results_path):
        ...
        print(results_path)
        print('-------------------------------------------------------------------------------------------------------')
        print('{:<50s}{:<15s}{:<15s}{:<15s}'.format('', 'Processed', 'Added', 'Skipped/Ignored'))
        print('-------------------------------------------------------------------------------------------------------')
        if results_path.startswith('s3://'):
            self.add_results_from_s3(results_path)
        else:
            for file in [f for f in listdir(results_path) if isfile(join(results_path, f))]:
    ...
    def add_result(self, result_content):
        ...
        if json_data['status'] in ['broken', 'failed'] or json_data['fullName'] in self.results:
            result = Result(json_data['status'], json_data['uuid'],
                            json_data['start'], json_data['stop'], json_data['statusDetails'].get('trace'))
        ...
        print('{:<50s}{:<15n}{:<15n}{:<15n}'.format('{0}-{1}'.format(json_data['uuid'], action), self.processed_files,
                                                    self.added_results,
                                                    self.skipped_results), end='\r')
    ...
    def create_insights(self):
        for test in self.results:
            ...
            self.results[test]['failure_rate'] = int(Decimal(failures / status_list.__len__()) * 100)
